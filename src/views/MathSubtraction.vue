<template>
  <div id="mathSubtration">
    <section class="portfolio d-print-none">
      <div class="container">
        <h2 class="page-section-heading text-center text-uppercase text-secondary mb-0">Math Subtraction</h2>
        <!-- Icon Divider-->
        ...
        <div id="no-print" class="row">
          <div class="col align-self-center">
            <div class="form-row">
              ...
              <MathFnButtons :pageId="'mathSubtration'" :components="['answer', 'generator', 'print', 'help']" v-on:show-answer="onShowAnswer" v-on:generate-values="handleGenerateValues"/>
            </div>
          </div>
        </div>
        <div class="row">
          <div class="col align-self-center">
            <h3>Subtracting numbers {{ fdigit }} digit(s) by {{ sdigit }} digit(s).</h3>
          </div>
        </div>
      </div>
    ...
    <div class="container" :class="hasAnswer ? 'printable-questions' : ''">
      <div class="row d-none d-print-inline">
        <div class="col text-center">
          <h1>Mathematics Subtraction - {{ fdigit }} digit(s) by {{ sdigit }} digit(s).</h1>
        </div>
      </div>
      <div class="row">
        <div class="col">
    ...
    <div class="container" v-show="hasAnswer" :class="hasAnswer ? 'printable-answers' : 'd-print-none'">
      <div class="row">
        <div class="col">
          <h3 class="is-size-6 text-dark">Anwsers.</h3>
        </div>
      </div>
      <div class="row">
        <div class="col text-justify d-print-col-bs">
          <ol>
            <li v-bind:key="index" v-for="(numValue, index) in numValues" class="d-print-col-bs">
              {{ numValue.answer }}
            </li>
          </ol>
        </div>
...
export default {
  ...
  methods: {
    ...
    getValues(fdigit, sdigit, cdigit, hasAnswer) {
      apiService
        .getBasicMathOperationDigits("subtract", fdigit, sdigit, cdigit, hasAnswer)
        .then((res) => {
